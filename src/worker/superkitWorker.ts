...
ctx.onmessage = (event: MessageEvent) => {
  ...
  const { kit1Text, kit2Text, options } = event.data as {
    kit1Text: string;
    kit2Text: string;
  ...
  try {
    ...
    const kit1SNPs = parseRawDnaText(kit1Text);
    ...
    const kit2SNPs = parseRawDnaText(kit2Text);
    ...
    // -------------------------------------------------------------
    // STAGE 5: Generating Output File Format
    // -------------------------------------------------------------
    postProgress('generating_output', 5, 90, `Formatting export file as ${options.outputFormat.toUpperCase()}...`);

    const outputLines: string[] = [];
    const timestamp = new Date().toISOString();

    if (options.outputFormat === 'ancestry') {
      outputLines.push('# AncestryDNA Raw Data SuperKit Export');
      outputLines.push(`# Generated by DNA SuperKit Builder on ${timestamp}`);
      outputLines.push('# Build Reference: GRCh37 (hg19)');
      outputLines.push(`# Total SuperKit Loci: ${finalSNPs.length.toLocaleString()}`);
      outputLines.push('# Format: rsid\tchromosome\tposition\tallele1\tallele2');
      outputLines.push('rsid\tchromosome\tposition\tallele1\tallele2');

      for (let i = 0; i < finalSNPs.length; i++) {
        const s = finalSNPs[i];
        outputLines.push(`${s.rsid}\t${s.chromosome}\t${s.position}\t${s.allele1}\t${s.allele2}`);
      }
    } else {
      // 23andMe format
      outputLines.push('# 23andMe Raw Data SuperKit Export');
      outputLines.push(`# Generated by DNA SuperKit Builder on ${timestamp}`);
      outputLines.push('# Assembly: GRCh37');
      outputLines.push(`# Total SuperKit Loci: ${finalSNPs.length.toLocaleString()}`);
      outputLines.push('# rsid\tchromosome\tposition\tgenotype');

      for (let i = 0; i < finalSNPs.length; i++) {
        const s = finalSNPs[i];
        const geno = `${s.allele1}${s.allele2}`;
        outputLines.push(`${s.rsid}\t${s.chromosome}\t${s.position}\t${geno}`);
      }
    }
